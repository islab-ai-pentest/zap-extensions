 ...
 */
package org.zaproxy.addon.cipher;
 ...
 */
public class ExtensionCipher extends ExtensionAdaptor {

    // The name is public so that other extensions can access it
    public static final String NAME = "ExtensionCipher";

    // The i18n prefix, by default the package name - defined in one place to make it easier
    // to copy and change this example
    protected static final String PREFIX = "cipher";
    ...
    private BrowserPanel statusPanel;

    private static final Logger LOGGER = LogManager.getLogger(ExtensionCipher.class);

    public ExtensionCipher() {
        super(NAME);
    ...
    public void hook(ExtensionHook extensionHook) {
        ...
        if (hasView()) {
            ...
            extensionHook.getHookView().addStatusPanel(getWebViewPanel());
        }
    }
    ...
    private AbstractPanel getWebViewPanel () {
        if (statusPanel == null) {
            statusPanel = new BrowserPanel(true);
    ...
    private void displayFile() {
        ...
        try {
            File f = new File(Constant.getZapHome(), ExtensionCipher.EXAMPLE_FILE);
            if (!f.exists()) {
                // This is something the user should know, so show a warning dialog
                View.getSingleton()
                        .showWarningDialog(
                                Constant.messages.getString(
                                        ExtensionCipher.PREFIX + ".error.nofile",
                                        f.getAbsolutePath()));
                return;
            }
